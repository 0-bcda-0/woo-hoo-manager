...
final class SSW_Stock_Ledger {
	...
	public static function paired_transfer_movements( $product_id, $from_location_id, $to_location_id, $quantity, $source_ref = '' ) {
		...
		return array(
			self::normalize_movement( array(
				'product_id' => $product_id, 'location_id' => $from_location_id, 'delta' => -$quantity,
				'type' => 'transfer_out', 'source' => 'transfer', 'source_ref' => $source_ref,
			) ),
			self::normalize_movement( array(
				'product_id' => $product_id, 'location_id' => $to_location_id, 'delta' => $quantity,
				'type' => 'transfer_in', 'source' => 'transfer', 'source_ref' => $source_ref,
			) ),
		);
	}
	 ...
	 *
	 * @param array $data Movement input.
	 * @param bool  $manage_transaction Whether this method owns START/COMMIT/ROLLBACK.
	 * @return array|WP_Error
	 */
	public static function record_movement( $data, $manage_transaction = true ) {
		global $wpdb;
		...
		return array( 'movement_id' => (int) $wpdb->insert_id, 'before' => $before, 'delta' => $movement['delta'], 'after' => $after );
	}
	...
	public static function transfer( $product_id, $from_location_id, $to_location_id, $quantity, $source_ref = '' ) {
		...
		$from = self::record_movement( $pair[0], false );
		if ( is_wp_error( $from ) ) { $wpdb->query( 'ROLLBACK' ); return $from; }
		$to = self::record_movement( $pair[1], false );
		if ( is_wp_error( $to ) ) { $wpdb->query( 'ROLLBACK' ); return $to; }
		$wpdb->query( 'COMMIT' );
