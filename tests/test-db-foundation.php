...
ssw_assert( '10' === (string) SSW_DB_SCHEMA_VERSION, 'Weekly report snapshots raise schema version to 10.' );

SSW_DB::maybe_upgrade();
ssw_assert( '10' === (string) get_option( 'ssw_db_schema_version', '0' ), 'First upgrade stores schema version 10.' );
ssw_assert( 20 === count( $ssw_test_dbdelta_calls ), 'First upgrade creates all approved foundation, bundle, alert and report tables.' );
...
ssw_assert( false !== strpos( $ssw_test_dbdelta_calls[16], 'ssw_bundle_product_orders' ), 'Bundle product counters migration is present.' );
...
ssw_assert( false !== strpos( $ssw_test_dbdelta_calls[18], 'ssw_alerts' ), 'Smart Alerts migration is present.' );
ssw_assert( false !== strpos( $ssw_test_dbdelta_calls[19], 'ssw_reports' ), 'Weekly report snapshot migration is present.' );

SSW_DB::maybe_upgrade();
ssw_assert( 20 === count( $ssw_test_dbdelta_calls ), 'Second upgrade is idempotent and does not rerun migrations.' );
