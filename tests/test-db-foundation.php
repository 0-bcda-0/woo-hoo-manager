...
ssw_assert('11'===(string)get_option('ssw_db_schema_version','0'),'Upgrade stores schema version 11.');ssw_assert(21===count($ssw_test_dbdelta_calls),'Schema v11 creates 21 active table definitions after barcode removal.');
$need=array('ssw_suppliers','ssw_supplier_products','ssw_locations','ssw_location_stock','ssw_stock_movements','ssw_purchase_orders','ssw_purchase_order_items','ssw_po_receipts','ssw_po_receipt_items','ssw_sales_daily','ssw_inventory_snapshots_daily','ssw_product_metrics_daily','ssw_forecasts','ssw_bundle_pairs','ssw_bundle_product_orders','ssw_bundle_processed_orders','ssw_alerts','ssw_bundles','ssw_bundle_components','ssw_reports');
...
foreach($ssw_test_dbdelta_calls as $sql){ssw_assert(false===strpos($sql,'ssw_barcodes'),'Removed barcode table is not created.');}
SSW_DB::maybe_upgrade();ssw_assert(21===count($ssw_test_dbdelta_calls),'Second upgrade is idempotent.');fwrite(STDOUT,"PASS: database migration foundation\n");
